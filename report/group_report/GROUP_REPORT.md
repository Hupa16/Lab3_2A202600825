...
## 7. How to run (instructor demo)

```bash
pip install -r requirements.txt
cp .env.example .env   # add API key

python run_chatbot.py "4-day Central Vietnam beach tour, 2M VND/day"
python run_agent.py --version v2
...
1. **Traces are the truth** — debugging v2 came from `logs/`, not guessing prompts.
2. **Tool design = API design** — clear parameter enums (`tier`, `region`) reduced agent errors more than longer prompts alone.
3. **Chatbot is enough for FAQ**; **ReAct is required** when users need consistent numbers and structured plans.
